...
def list_videos_featured(channel_url):
	...
	for video in featured_videos['videos']:
		# Fetch video's metadata from the video's page
		meta_data = full30.get_video_metadata(video['hash'])
		...
		list_item.setArt(
			{
				...
				'fanart' : featured_videos['metadata']['image']
			})
		...
		list_item.setArt(
			{
				'thumb' : video['thumbnail'],
				'poster' : video['thumbnail'],
				'fanart' : recent_videos['metadata']['image']
			})
